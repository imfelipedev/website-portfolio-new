...
export default function Project(props: Props) {
    return (
        <div className="flex flex-col items-start justify-center rounded-2xl overflow-hidden">
            ...
            <p className="mt-10 text-[2.4rem] text-[#fafafa] font-semibold">{props.name}</p>

            <p className="mt-5 text-[1.8rem] text-[#a1a1a1]">{props.description}</p>

            <div className="mt-5 flex gap-5">
                {props.technologies.map((technology, index) => (
                    <span key={index} className="px-5 py-2 rounded-xl bg-[#1f1f1f] text-[#fafafa] text-[1.4rem]">
                        {technology}
                    </span>
                ))}
            </div>
